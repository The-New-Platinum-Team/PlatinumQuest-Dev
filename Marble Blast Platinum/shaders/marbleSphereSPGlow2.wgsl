...
fn fMarble(in: TSRasterizerData) -> @location(0) vec4<f32> {
   ...
   var desired: vec3<f32> = (transpose(fourMat) * vec4<f32>(0, 1, 0, 0)).xyz;
   var glowFactor: f32 = 0.3 + 0.7 * pow(dot(desired, in.camDir.xyz), 2.0);


   var calcColor : vec4<f32> = vec4<f32>(mix(mix(materialColor.rgb * in.color.rgb, reflectionColor.rgb, reflectAmount),
                 materialColor.rgb * glowFactor, materialColor.a),
             1);
   return calcColor;
}