...
fn fMarble(in: TSRasterizerData) -> @location(0) vec4<f32> {
   ...
   var reflectAmount = invSigmoid(0.01 + 0.98 * materialColor.a);
   reflectAmount -= 0.7 * (2.0 * -dot(in.norm, in.camDir.xyz) - 1.0);
   reflectAmount = sigmoid(reflectAmount);
   reflectAmount = 0.95 * reflectAmount;

   var calcColor : vec4<f32> = vec4<f32>(mix(materialColor.rgb * in.color.rgb, reflectionColor.rgb, reflectAmount), 1.0);
