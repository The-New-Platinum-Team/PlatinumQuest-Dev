...
fn fMarble(in: TSRasterizerData) -> @location(0) vec4<f32> {
   ...
   var refraction1 : vec3<f32> = normalize(refract(in.camDir.xyz, in.norm, 1.0 / ior));
   ...
   var outNormal : vec3<f32> = normalize(in.norm + t * refraction1);
   ...
   refractAmount += 0.7 * dot(in.norm, in.camDir.xyz);
