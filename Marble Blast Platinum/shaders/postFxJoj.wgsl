...
fn fPostFx(in: RasterizerData) -> @location(0) vec4<f32> {
   var sampleUV: vec2<f32> = in.uv1;
   if (sampleUV.x > 0.5 * commonUniforms.miscControl.x) {
      sampleUV = vec2<f32>(commonUniforms.miscControl.x - sampleUV.x, sampleUV.y);
   }
   return textureSample(texs0, samplers0, sampleUV).rgba * in.color;
}