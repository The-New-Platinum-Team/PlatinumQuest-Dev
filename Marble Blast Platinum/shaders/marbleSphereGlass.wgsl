...
fn wavy(v: vec3<f32>) -> vec3<f32> {
    return sin(20.0 * v * vec3<f32>(1.0, 0.2, 0.4)) + 
           cos(23.0 * v * vec3<f32>(-0.5, 0.3, 0.8)) +
           sin(16.0 * v * vec3<f32>(-0.1, 0.9, 0.2));
}
...
fn normalWarp(normal: vec3<f32>) -> vec3<f32> {
    ...
    return (transpose(fourMat) * 
           vec4<f32>(normalize(local + 0.1 * wavy(local) + 
           0.03 * vec3<f32>(GetRandom(local.yz), GetRandom(local.zx), GetRandom(local.xy))), 0.0)).xyz;
}
...
fn fMarble(in: TSRasterizerData) -> @location(0) vec4<f32> {
   ...
   var materialColor : vec4<f32> = textureSample(texs0, samplers0, in.uv1);

   var normalModel2 = normalWarp(in.norm);

   var cameraReflection: vec3<f32> = reflect(in.camDir.xyz, normalModel2);
   
   var reflectAmount : f32 = materialColor.a * 0.3;
   var refractAmount : f32 = materialColor.a;

   var ior : f32 = 1.3;  // This is actually water, not glass, but glass doesn't look as sexy
   var refraction1 : vec3<f32> = normalize(refract(in.camDir.xyz, normalModel2, 1.0 / ior));
   
   var t : f32 = (-dot(in.norm, refraction1) +
               sqrt(dot(refraction1, refraction1) -
                    dot(cross(in.norm, refraction1), cross(in.norm, refraction1)))) /
              dot(refraction1, refraction1);
   var outNormal : vec3<f32> = normalWarp(normalize(in.norm + t * refraction1));
   var refraction2 : vec3<f32> = refract(refraction1, -outNormal, ior);
   var refractionColor = textureSample(cubeTex, cubeSampler, refraction2.xzy);
   refractAmount = invSigmoid(0.01 + 0.98 * refractAmount);
   refractAmount += 0.7 * dot(normalModel2, in.camDir.xyz);
   refractAmount = sigmoid(refractAmount);

   var backColor = textureSample(texs0, samplers0, uvFromNormal(outNormal));
   refractionColor =
        mix(vec4<f32>(backColor.rgb * in.color.rgb, 1), refractionColor, backColor.a);
   ...
   var calcColor : vec4<f32> = vec4<f32>(mix(mix(materialColor.rgb * in.color.rgb, refractionColor.rgb, refractAmount), reflectionColor.rgb, reflectAmount), 1.0);
